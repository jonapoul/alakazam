...
fun PublishingExtension.addPublication(project: Project, artifact: String, artifactType: ArtifactType) {
  publications {
    create<MavenPublication>(name = "maven") {
      groupId = "dev.jonpoulton.alakazam"
      artifactId = artifact
      version = BuildConstants.LIB_VERSION_NAME
      from(project.components.getByName(artifactType.componentName))
