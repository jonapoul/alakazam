 ...
 * ```
 *
 * [delegate] is only used for its descriptor, and defaults to the enum's generated serializer. So
 * [E] itself must not be annotated with `@Serializable(with = ...)` pointing back at this
...
internal class FallbackEnumSerializer<E : Enum<E>>(
  ...
  override fun deserialize(decoder: Decoder): E {
    // Formats are inconsistent here. Some throw for an unknown entry, others report an
    // out-of-bounds index.
    val index =
      try {
