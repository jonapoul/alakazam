...
internal class FlowOnEachOfTypeTest {
  ...
  fun `String flow doesn't emit any ints`() = runTest {
    ...
    flowOf("a", "b", "c").onEachOfType<String, Int> { grabbedInts.add(it) }.collect()
  ...
  fun `Int flow doesn't emit any strings`() = runTest {
    ...
    flowOf(1, 2, 3).onEachOfType<Int, String> { grabbedStrings.add(it) }.collect()
...
internal class FlowTakeUntilExclusiveTest {
  ...
  fun `No matches doesn't cancel the flow`() = runTest {
    val taken = flowOf("a", "b", "c").takeUntilExclusive { it == "d" }.toList()
    assertEquals(3, taken.size)
  }

  @Test
  fun `Matching the last emission excludes it`() = runTest {
    val taken = flowOf("a", "b", "c").takeUntilExclusive { it == "c" }.toList()
    assertArrayEquals(
      arrayOf("a", "b"),
      taken.toTypedArray(),
    )
  ...
  fun `Matching the first emission returns empty list`() = runTest {
    val taken = flowOf("a", "b", "c").takeUntilExclusive { it == "a" }.toList()
    assertArrayEquals(
      arrayOf(),
      taken.toTypedArray(),
    )
...
internal class FlowTakeUntilInclusiveTest {
  ...
  fun `No matches returns all emissions`() = runTest {
    val taken = flowOf("a", "b", "c").takeUntilInclusive { it == "d" }.toList()
    assertArrayEquals(
      arrayOf("a", "b", "c"),
      taken.toTypedArray(),
    )
  ...
  fun `Matching the last emission includes it`() = runTest {
    val taken = flowOf("a", "b", "c").takeUntilInclusive { it == "c" }.toList()
    assertArrayEquals(
      arrayOf("a", "b", "c"),
      taken.toTypedArray(),
    )
  ...
  fun `Matching the first emission returns first element`() = runTest {
    val taken = flowOf("a", "b", "c").takeUntilInclusive { it == "a" }.toList()
    assertArrayEquals(
      arrayOf("a"),
      taken.toTypedArray(),
    )
